...
def _sanitize_video_components(messages: list[dict]) -> None:
    """Replace unsupported <Video> components with a <Text> component containing the video URL, in place."""
    for m in messages:
        surface = m.get("surfaceUpdate")
        if not isinstance(surface, dict):
            continue
        for c in surface.get("components") or []:
            if not isinstance(c, dict):
                continue
            comp = c.get("component")
            if not isinstance(comp, dict) or "Video" not in comp:
                continue
            vid = comp.get("Video")
            url = vid.get("url") if isinstance(vid, dict) else None
            literal = url.get("literalString") if isinstance(url, dict) else ""
            c["component"] = {
                "Text": {
                    "text": {"literalString": f"🎥 Watch Exercise Video: {literal}" if literal else "🎥 Exercise Video generated."},
                    "usageHint": "body",
                }
            }
...
def _ensure_begin_rendering(messages: list[dict]) -> list[dict]:
    """Ensure every surfaceUpdate message has a preceding beginRendering message."""
    has_begin = set()
    for m in messages:
        br = m.get("beginRendering")
        if isinstance(br, dict):
            s_id = br.get("surfaceId") or m.get("surfaceId")
            if s_id:
                has_begin.add(s_id)
    ...
    for m in messages:
        ...
        if isinstance(su, dict):
            s_id = su.get("surfaceId") or m.get("surfaceId")
            if s_id and s_id not in has_begin:
                components = su.get("components") or []
                ids, refs = _component_ids_and_refs(components)
                roots = ids - refs
                root_id = list(roots)[0] if roots else (components[0].get("id") if components else "root")
...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        # Auto-inject missing beginRendering message if model emitted only surfaceUpdate
        ...
        # Convert unsupported <Video> components into <Text> with public URL.
        _sanitize_video_components(messages)
